...
它只带来一条新规矩：**宁可写清证据只到哪一步、窗口为什么还早，也不许为了凑数硬夸。**
交「今天没有值得展开的」等于判断失败，不是合法交卷——采集网开着的日子里，空简报说明过滤没看见，不是市场没有东西。
页面质量检查失败也要先出站，任务变红；不能因为一道门禁让日期停在昨天。
...
**本期摘要的来源署名例外（用户要求）：可展示真实媒体、公告或数据发布方名称，并提供核验链接；英文页保留来源专名。来源必须来自原始记录，不能由模型编造。**

**其他正文不出现内部采集源名称。** 用户不关心东西从哪抓来的 —— 那是实现细节，
...
- GitHub 发现不能只靠自然语言关键词：`sources.yaml` 中的专题、官方组织和突破项目通道
  是独立兜底。专题只保证发现完整性；官方或突破通道标记的 `priority_review: true` 产品不得被
  判断层淘汰，且必须同时出现于中英文日报。
