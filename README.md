...
两步现在都接了定时，你不用说话也会更新：采集跑在 GitHub Actions 上，判断跑在云端
Claude routine 上，早上七点打开站就是当天写好的。上面那三条命令一直有效，
...
| 时间 | 谁在跑 | 干什么 |
|------|--------|--------|
| **06:00** | GitHub Actions（`.github/workflows/daily.yml`） | 采集 → 清理过老存档 → 生成网站 → 提交推送 |
| **07:00** | 云端 Claude routine「x-octo 每日判断层」 | 过滤 → 深度分析 → 写中英文简报 → 复算约束 → 提交推送 |

推送完 Vercel 自动部署，所以你七点多打开站，看到的是当天已经写好的简报。

**为什么错开一小时**：GitHub 的定时任务经常迟到（实测过迟两个多小时），
判断层开跑时要是采集还没落地，就会对着空气写简报。所以判断层第一件事是查
当天采到没有，没有就自己补跑一遍采集 —— 一小时余量加上这个兜底，两层保险。

**为什么是 6 点不是 9 点**：读者要在 7 点看到成品，采集必须赶在写简报之前。
代价是 6 点对应美西前一天下午 3 点，Product Hunt 当天的榜单还差几个小时收摊，
票数比原来 9 点那版更「生」一点。

想立刻跑一次：采集去 GitHub 仓库的 Actions 页面点「每日采集」的 Run workflow；
判断层去 https://claude.ai/code/routines 点那条 routine 的运行。

**没有人会复核**。判断层写完直接上线，写歪了也会直接上站，你事后才能改。
这是拿「每天准时有」换「每天有人把关」，想换回来就把 routine 改成开 PR 而不是直接 push。
