...
**第二步，让 Codex（或手动用 Claude Code）过滤和分析**：

自动任务会读 `config/filter.md` 的规则把噪音砍掉，
...
所以采集改在 06:17。判断层开跑时先查当天存档；没采到就自己补跑采集，已看到 GitHub
任务仍在跑则先等它结束。GitHub 预采集加 Codex 兜底，两层保险。
