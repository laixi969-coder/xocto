...
| 9 | 内部闸门归一化被误报为首页“重要更新” | MEDIUM | `src/xocto/req_review.py` | 仅当最终结论或连续通过的 REQ 阶段真正变化时生成事件，并展示当前关键闸门依据 |
...
停止继续微调字符数。重新定义完整判断的质量边界为四个可验证语义不变量：闸门必须按 REQ 顺序且先拦后续；supported/challenged 必须引用本项目公开证据；活动闸门不得复制同一理由；下一项验证必须指向 xOcto 可继续追踪的公开证据渠道，不能转交网站读者。真实模型仍会偶发违反这些约束，因此最终边界改为“模型提出、程序保守归一化”：无证据支持、阶段穿透、短理由、重复理由和读者委派会被逐项降级或替换，不再让单条内容偏差导致整批研究回滚。

架构修复提交 `37a58fc6` 在 GitHub Actions `32688398391` 通过真实模型与完整发布链，并写回 15 个返工后的完整判断。最终抽查清除了由内部归一化产生的伪“重要更新”，本地 95 项测试、完整建站和七项发布门槛再次全绿；审计达到收敛条件。
