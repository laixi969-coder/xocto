...
停止继续微调字符数。重新定义完整判断的质量边界为四个可验证语义不变量：闸门必须按 REQ 顺序且先拦后续；supported/challenged 必须引用本项目公开证据；活动闸门不得复制同一理由；下一项验证必须指向 xOcto 可继续追踪的公开证据渠道，不能转交网站读者。真实模型仍会偶发违反这些约束，因此最终边界改为“模型提出、程序保守归一化”：无证据支持、阶段穿透、短理由、重复理由和读者委派会被逐项降级或替换，不再让单条内容偏差导致整批研究回滚。完成真实模型返工后再宣告整个审计收敛。
