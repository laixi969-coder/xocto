---
slug: shape
name: Shape
builder: nathan
category: AI + 开发
summary_zh: Shape 定位为面向设计师和程序员的 agentic IDE，但具体工作流程、输入输出和交付物均未在公开材料中说明，需进一步核验。
inspiration: 趋势是 IDE 从编辑器转向 agent 协作。但切入需明确具体设计或开发环节，否则容易沦为通用工具。
summary_en: Shape is positioned as an agentic IDE for designers and programmers, but specific workflows,
  inputs, outputs, and deliverables are not described in public materials and need further verification.
inspiration_en: The trend is IDE shifting from editor to agent collaboration. But entry needs to target
  a specific design or development phase, otherwise it risks becoming a generic tool.
priority_review: false
project_type: new_application
industries:
- 软件开发
- 设计
industries_en:
- Software Development
- Design
jobs:
- 设计师
- 程序员
jobs_en:
- Designer
- Programmer
regions: []
regions_en: []
open_source: false
url: https://www.producthunt.com/products/shape-5
canonical_url: https://producthunt.com/products/shape-5
summary: The agentic IDE for designers and programmers
first_seen: '2026-08-19T23:56:11Z'
last_seen: '2026-08-29T15:06:37Z'
status: watching
sources:
- producthunt
- newssearch
sightings:
- source: producthunt
  url: https://www.producthunt.com/products/shape-5
  seen_at: '2026-08-20T22:45:15Z'
  metrics: {}
  kind: product
- source: newssearch
  url: https://news.google.?oc=5
  seen_at: '2026-08-29T15:06:37Z'
  metrics: {}
  kind: news
---

# Shape

The agentic IDE for designers and programmers

## 笔记


