---
slug: shape
name: Shape
builder: nathan
category: AI + 开发
summary_zh: 设计师与程序员在进行界面与代码协同开发时，可在该集成开发环境中操作；目前公开资料仅将其定义为面向两类人群的智能体 IDE，具体输入格式、AI 自动化动作与最终交付产物仍待进一步公开与核验。
inspiration: 趋势是从纯代码生成走向设计与开发协同的智能体环境。切入点在于打通设计稿组件与前端工程代码的双向实时同步，解决两套资产脱节的摩擦。
summary_en: Shape is positioned as an agentic IDE for designers and programmers to collaborate on interface
  and code, though specific input formats, agent workflows, and deliverables remain to be verified from
  public materials.
inspiration_en: The trend is moving toward unified agentic environments bridging design and code. An effective
  entry point is real-time synchronization between visual design components and frontend codebases.
priority_review: false
project_type: new_application
industries:
- 软件开发
- 创意设计
industries_en:
- Software Development
- Creative Design
jobs:
- 软件工程师
- UI/UX设计师
jobs_en:
- Software Engineer
- UI/UX Designer
regions:
- 全球
regions_en:
- Global
open_source: false
url: https://www.producthunt.com/products/shape-5
canonical_url: https://producthunt.com/products/shape-5
summary: The agentic IDE for designers and programmers
first_seen: '2026-08-19T23:56:11Z'
last_seen: '2026-09-08T14:34:47Z'
status: rejected
sources:
- producthunt
- newssearch
- marketfeeds
sightings:
- source: producthunt
  url: https://www.producthunt.com/products/shape-5
  seen_at: '2026-08-20T22:45:15Z'
  metrics: {}
  kind: product
- source: newssearch
  url: https://news.google.?oc=5
  seen_at: '2026-08-29T15:06:37Z'
  metrics: {}
  kind: news
- source: newssearch
  url: https://news.google.com/rss/articles/CBMikAFBVV95cUxNcDVRVzBRak1PdzB5dWtZc0pKU29LRWJHUW45YThFZTFvZzBjSkoteDZLVElxRkR4a0pGSWxXU181d1VESHZkTUFKQmZiYnhHU2pYVDhiZ01DTHhNa1U3YzFEUlFPZ0p6bl95cXk0SU5VMFZvV0pQRV83VWxMZlM4Y0R0VFNDMzhjX0YxUmItMXo?oc=5
  seen_at: '2026-09-06T04:01:10Z'
  metrics: {}
  kind: news
- source: marketfeeds
  url: https://tech.eu/2026/09/08/tom-noble-is-building-his-business-in-public-and-letting-the-internet-shape-what-comes-next/
  seen_at: '2026-09-08T14:34:47Z'
  metrics: {}
  kind: news
---

# Shape

The agentic IDE for designers and programmers

## 笔记


