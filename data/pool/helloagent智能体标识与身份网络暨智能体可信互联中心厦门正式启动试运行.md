---
slug: helloagent智能体标识与身份网络暨智能体可信互联中心厦门正式启动试运行
name: 智能体标识与身份网络暨智能体可信互联中心（厦门）
builder: ''
category: ''
summary_zh: 2026年9月22日，智能体标识与身份网络暨智能体可信互联中心（厦门）启动试运行，为智能体提供标识与身份互认基础设施。这意味着智能体跨平台可信互联开始有公共基础设施支撑，对 AI
  应用的身份认证、互操作与合规交付构成新的外部条件（推断）。
inspiration: ''
summary_en: On September 22, 2026, the Agent Identifier and Identity Network and the Trusted Agent Interconnection
  Center (Xiamen) began trial operation, providing identifier and identity infrastructure for agents.
  This means trusted cross-platform agent interconnection is beginning to gain public infrastructure support,
  creating new external conditions for identity authentication, interoperability and compliant delivery
  of AI applications (inference).
inspiration_en: ''
priority_review: false
project_type: new_application
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://news.google.com/rss/articles/CBMiaEFVX3lxTE9vVFlXdk5ZWlczSldQdTQ3QzFVYnVCN3NZbHI1cC1RalNZNzN5TDE4NG0teS1JVmRoVFpmMWVaNFVBZXI2NmU3dTdPZ0lhanZmR1ZLSjZlWm00VW55S3pNUWVaNFJubnZ2?oc=5
canonical_url: https://news.google.com/rss/articles/CBMiaEFVX3lxTE9vVFlXdk5ZWlczSldQdTQ3QzFVYnVCN3NZbHI1cC1RalNZNzN5TDE4NG0teS1JVmRoVFpmMWVaNFVBZXI2NmU3dTdPZ0lhanZmR1ZLSjZlWm00VW55S3pNUWVaNFJubnZ2?oc=5
summary: Hello,Agent——智能体标识与身份网络暨智能体可信互联中心（厦门）正式启动试运行 rmzxw.com.cn
first_seen: '2026-09-22T02:34:00Z'
last_seen: '2026-09-25T00:34:18Z'
status: pending_filter
sources:
- newssearch
sightings:
- source: newssearch
  url: https://news.google.com/rss/articles/CBMiaEFVX3lxTE9vVFlXdk5ZWlczSldQdTQ3QzFVYnVCN3NZbHI1cC1RalNZNzN5TDE4NG0teS1JVmRoVFpmMWVaNFVBZXI2NmU3dTdPZ0lhanZmR1ZLSjZlWm00VW55S3pNUWVaNFJubnZ2?oc=5
  seen_at: '2026-09-23T00:34:44Z'
  metrics: {}
  kind: news
- source: newssearch
  url: https://news.google.com/rss/articles/CBMib0FVX3lxTE1XbFQ2c3V5eWJpSjVVMEQtYUFzTVVsQ2hoRDV5aDBibDhoMC0yeWtIZ0dWenBjcWJydFRlMnI0SDlQSnllUmNuMl8tTlBUOUNETGlDb2JBaDNSY1V2TnZoOVp2ZnFoWk03M0dZdWppTQ?oc=5
  seen_at: '2026-09-25T00:34:18Z'
  metrics: {}
  kind: news
---

# 智能体标识与身份网络暨智能体可信互联中心（厦门）

Hello,Agent——智能体标识与身份网络暨智能体可信互联中心（厦门）正式启动试运行 rmzxw.com.cn

## 笔记


