---
slug: agentsight
name: AgentSight
builder: forrestly
category: 基础层
summary_zh: 基于eBPF的AI智能体可观测性工具，无需改代码。
inspiration: 可观测性是智能体运维的盲区，无侵入方案有迁移价值。
summary_en: eBPF-based observability for AI agents, no code changes.
inspiration_en: Observability is a blind spot in agent ops; non-invasive solutions are portable.
priority_review: false
url: https://github.com/alibaba/anolisa/blob/main/docs/user-guide/en/agent-observability/agentsight.md
canonical_url: https://github.com/alibaba/anolisa/blob/main/docs/user-guide/en/agent-observability/agentsight.md
summary: eBPF observability for AI agents, no code changes
first_seen: '2026-08-21T15:21:10Z'
last_seen: '2026-08-21T22:41:50Z'
status: queued
sources:
- hackernews
sightings:
- source: hackernews
  url: https://github.com/alibaba/anolisa/blob/main/docs/user-guide/en/agent-observability/agentsight.md
  seen_at: '2026-08-21T22:41:50Z'
  metrics:
    points: 14
    comments: 0
---

# AgentSight

eBPF observability for AI agents, no code changes

## 笔记


