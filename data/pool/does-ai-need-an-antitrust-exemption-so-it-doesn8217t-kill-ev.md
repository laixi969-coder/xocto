---
slug: does-ai-need-an-antitrust-exemption-so-it-doesn8217t-kill-ev
name: Does AI need an antitrust exemption so it doesn&#8217;t kill everyone????
builder: ''
category: ''
summary_zh: ''
inspiration: ''
summary_en: ''
inspiration_en: ''
priority_review: false
project_type: new_application
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://www.theverge.com/podcast/997382/openai-microsoft-anthropic-elon-musk-cartel-ai-competition
canonical_url: https://theverge.com/podcast/997382/openai-microsoft-anthropic-elon-musk-cartel-ai-competition
summary: Today on Decoder, we’ve got the first of a two-part series on the future of business, and I’m
  talking with Jonathan Kanter, the former antitrust chief for the US Department of Justice in the Biden
  administration. These days, he’s both a professor of law at WashU and professor of technology policy
  at Carnegie Mellon. The […]
first_seen: '2026-09-19T14:00:00Z'
last_seen: '2026-09-20T00:09:45Z'
status: rejected
sources:
- marketfeeds
sightings:
- source: marketfeeds
  url: https://www.theverge.com/podcast/997382/openai-microsoft-anthropic-elon-musk-cartel-ai-competition
  seen_at: '2026-09-20T00:09:45Z'
  metrics: {}
  kind: news
---

# Does AI need an antitrust exemption so it doesn&#8217;t kill everyone????

Today on Decoder, we’ve got the first of a two-part series on the future of business, and I’m talking with Jonathan Kanter, the former antitrust chief for the US Department of Justice in the Biden administration. These days, he’s both a professor of law at WashU and professor of technology policy at Carnegie Mellon. The […]

## 笔记


