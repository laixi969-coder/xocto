---
slug: german-spacetech-isar-aerospaces-second-rocket-launch-reache
name: Isar Aerospace
builder: mpweiher
category: ''
summary_zh: Isar Aerospace 是德国商业运载火箭公司，其火箭在第二次飞行中进入轨道并成功部署有效载荷，公司称这是欧洲航天史的里程碑。这表明欧洲商业火箭已具备入轨与载荷部署能力，对欧洲建立本土发射运力是实质进展。
inspiration: ''
summary_en: Isar Aerospace, a German commercial launch company, reached orbit and deployed payloads on
  its second flight, which the company calls a milestone for European spaceflight. It shows European commercial
  rockets can now deliver payloads to orbit, a concrete step toward homegrown European launch capacity.
inspiration_en: ''
priority_review: false
project_type: new_application
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://sifted.eu/articles/isar-aerospace-test-flight-orbit/
canonical_url: https://sifted.eu/articles/isar-aerospace-test-flight-orbit
summary: Isar Aerospace reaches orbit and deploys payloads on second flight
first_seen: '2026-09-05T21:49:26Z'
last_seen: '2026-09-06T23:54:18Z'
status: pending_filter
sources:
- marketfeeds
- hackernews
sightings:
- source: marketfeeds
  url: https://sifted.eu/articles/isar-aerospace-test-flight-orbit/
  seen_at: '2026-09-05T23:54:22Z'
  metrics: {}
  kind: news
- source: hackernews
  url: https://isaraerospace.com/press/history-for-european-spaceflight-isar-aerospace-reaches-orbit-and-deploys-payloads-on-second-flight
  seen_at: '2026-09-06T23:54:18Z'
  metrics:
    points: 550
    comments: 178
  kind: news
---

# Isar Aerospace

Isar Aerospace reaches orbit and deploys payloads on second flight

## 笔记


