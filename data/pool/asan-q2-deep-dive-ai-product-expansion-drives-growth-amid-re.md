---
slug: asan-q2-deep-dive-ai-product-expansion-drives-growth-amid-re
name: Asana
builder: ''
category: ''
summary_zh: Asana 是工作管理平台，团队用它跟踪项目进度和任务分配。本次是财报分析，反映其 AI 功能扩展带来的增长与留存挑战，不构成独立新产品。
inspiration: ''
summary_en: Asana is a work management platform for teams to track projects and tasks. This is an earnings
  analysis showing AI expansion growth and retention challenges, not a new product.
inspiration_en: ''
priority_review: false
project_type: new_application
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://news.google.?oc=5
canonical_url: https://news.google.?oc=5
summary: 'ASAN Q2 Deep Dive: AI Product Expansion Drives Growth Amid Retention and PLG Headwinds TradingView'
first_seen: '2026-09-04T13:21:07Z'
last_seen: '2026-09-17T00:33:01Z'
status: rejected
sources:
- newssearch
- marketfeeds
sightings:
- source: newssearch
  url: https://news.google.?oc=5
  seen_at: '2026-09-06T15:20:04Z'
  metrics: {}
  kind: news
- source: marketfeeds
  url: https://www.theverge.com/ai-artificial-intelligence/995348/ai-dating-app-scams
  seen_at: '2026-09-17T00:33:01Z'
  metrics: {}
  kind: news
---

# Asana

ASAN Q2 Deep Dive: AI Product Expansion Drives Growth Amid Retention and PLG Headwinds TradingView

## 笔记


