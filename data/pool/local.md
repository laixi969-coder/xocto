---
slug: local
name: Local
builder: Lukas Wesemann
category: AI + 效率
summary_zh: Mac上零摩擦的本地AI工具。
inspiration: 本地AI的易用性突破，可迁移到其他本地优先应用。
summary_en: Zero friction local AI for your Mac.
inspiration_en: Usability breakthrough in local AI, applicable to other local-first apps.
priority_review: false
project_type: ''
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://www.producthunt.com/products/local-7
canonical_url: https://producthunt.com/products/local-7
summary: Zero (!) friction local AI for your Mac
first_seen: '2026-08-20T01:43:17Z'
last_seen: '2026-08-29T03:43:29Z'
status: pending_filter
sources:
- producthunt
- officialfeeds
- marketfeeds
sightings:
- source: producthunt
  url: https://www.producthunt.com/products/local-7
  seen_at: '2026-08-21T22:41:50Z'
  metrics: {}
  kind: product
- source: officialfeeds
  url: https://aws.amazon.com/blogs/machine-learning/introducing-openai-models-on-amazon-bedrock-for-in-country-inferencing-in-india/
  seen_at: '2026-08-29T03:43:28Z'
  metrics: {}
  kind: news
- source: officialfeeds
  url: https://aws.amazon.com/blogs/machine-learning/bring-your-own-model-with-amazon-sagemaker-ai-script-mode-in-sdk-v3/
  seen_at: '2026-08-29T03:43:28Z'
  metrics: {}
  kind: news
- source: marketfeeds
  url: https://arstechnica.com/ai/2026/08/ibms-new-granite-4-2-models-ride-the-wave-of-interest-in-local-llms/
  seen_at: '2026-08-29T03:43:29Z'
  metrics: {}
  kind: news
---

# Local

Zero (!) friction local AI for your Mac

## 笔记


