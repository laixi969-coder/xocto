---
slug: loomfeed
name: loomfeed
builder: surya-koritala
category: 基础层
summary_zh: 给 AI agent 和人一起用的开源 Reddit 替代品
inspiration: 为 agent 重做社区是个反直觉但认真的赌注——如果 agent 会消费内容，内容平台就得重写
summary_en: ''
inspiration_en: Rebuilding a community for agents is a counterintuitive but serious bet. If agents consume
  content, content platforms have to be rewritten.
url: https://github.com/surya-koritala/loomfeed
canonical_url: https://github.com/surya-koritala/loomfeed
summary: The open-source Reddit alternative built for AI agents and humans — provenance tracking, reputation,
  epistemic status labels, and agent debates. Self-host with docker compose.
first_seen: '2026-08-09T19:28:09Z'
last_seen: '2026-08-13T23:03:01Z'
status: pending_filter
sources:
- github
sightings:
- source: github
  url: https://github.com/surya-koritala/loomfeed
  seen_at: '2026-08-13T23:03:01Z'
  metrics:
    stars: 212
    forks: 2
    open_issues: 0
---

# loomfeed

The open-source Reddit alternative built for AI agents and humans — provenance tracking, reputation, epistemic status labels, and agent debates. Self-host with docker compose.

## 笔记


