---
slug: harvard-law-dropout-raises-6m-for-blue-voice-to-build-a-harv
name: Blue Voice
builder: ''
category: AI + 生活
summary_zh: Blue Voice 是一款面向警察的 AI 助手，旨在提供法律和程序指导，类似法律领域的 Harvey。已筹集 600 万美元，但具体功能和工作流程仍待核验。
inspiration: 趋势是 AI 垂直应用于公共安全。切入可从为执法人员提供实时法律咨询和报告生成，但需考虑伦理和合规风险。
summary_en: Blue Voice is an AI assistant for police officers, aiming to provide legal and procedural
  guidance, similar to Harvey in legal field. Raised $6M, but specific features and workflows remain to
  be verified.
inspiration_en: Trend is AI vertical application in public safety. Entry point could be providing real-time
  legal advice and report generation for law enforcement, but ethical and compliance risks need consideration.
priority_review: false
project_type: new_application
industries:
- 公共安全
- 法律
industries_en:
- Public Safety
- Legal
jobs:
- 警察
- 执法人员
jobs_en:
- Police officers
- Law enforcement
regions:
- 美国
regions_en:
- United States
open_source: false
url: https://news.google.?oc=5
canonical_url: https://news.google.?oc=5
summary: Harvard Law dropout raises $6M for Blue Voice to build a ‘Harvey for police officers’ TechCrunch
first_seen: '2026-08-31T18:35:11Z'
last_seen: '2026-09-01T01:18:43Z'
status: queued
sources:
- newssearch
sightings:
- source: newssearch
  url: https://news.google.?oc=5
  seen_at: '2026-09-01T01:18:43Z'
  metrics: {}
  kind: news
---

# Blue Voice

Harvard Law dropout raises $6M for Blue Voice to build a ‘Harvey for police officers’ TechCrunch

## 笔记


