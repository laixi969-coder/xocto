---
slug: langchain-ai-langchain-langchain-core161
name: LangChain
builder: langchain-ai
category: ''
summary_zh: LangChain 是一个开源框架，用于构建基于大语言模型的应用。本次发布是核心库的维护更新，属于开发者基础设施的常规迭代。
inspiration: ''
summary_en: LangChain is an open-source framework for building applications based on large language models.
  This release is a routine maintenance update to the core library, part of developer infrastructure evolution.
inspiration_en: ''
priority_review: false
project_type: open_source
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: true
url: https://github.com/langchain-ai/langchain/releases/tag/langchain-core%3D%3D1.6.1
canonical_url: https://github.com/langchain-ai/langchain/releases/tag/langchain-core%3D%3D1.6.1
summary: 'Changes since langchain-core==1.6.0


  revert: release(core): 1.6.2 (#39971)

  release(core): 1.6.2 (#39967)

  fix(core): shore up indexing in genai v1 streaming content (#39964)

  fix(core): make `StructuredTool` JSON-serializable (#39631)

  chore(deps): bump minor and patch dependencies (#39869)

  release(core): 1.6.1 (#39832)

  feat(core): propagate gateway information on error path (#39829)'
first_seen: '2026-08-27T19:31:34Z'
last_seen: '2026-09-22T00:50:25Z'
status: rejected
sources:
- github
sightings:
- source: github
  url: https://github.com/langchain-ai/langchain/releases/tag/langchain-core%3D%3D1.6.1
  seen_at: '2026-08-29T15:06:08Z'
  metrics:
    reactions: 0
  kind: news
- source: github
  url: https://github.com/langchain-ai/langchain/releases/tag/langchain-core%3D%3D1.6.2
  seen_at: '2026-09-05T23:54:04Z'
  metrics:
    reactions: 5
  kind: news
- source: github
  url: https://github.com/langchain-ai/langchain/releases/tag/langchain-core%3D%3D1.6.4
  seen_at: '2026-09-22T00:50:25Z'
  metrics:
    reactions: 0
  kind: news
---

# LangChain

Changes since langchain-core==1.6.0

revert: release(core): 1.6.2 (#39971)
release(core): 1.6.2 (#39967)
fix(core): shore up indexing in genai v1 streaming content (#39964)
fix(core): make `StructuredTool` JSON-serializable (#39631)
chore(deps): bump minor and patch dependencies (#39869)
release(core): 1.6.1 (#39832)
feat(core): propagate gateway information on error path (#39829)

## 笔记


