---
slug: ft
name: Flock
builder: sbulaev
category: ''
summary_zh: Flock 是一家提供 AI 监控系统的公司，其产品被美国多地的执法机构使用。此次事件是围绕该系统的政治和社会争议，属于行业监管与政策背景。
inspiration: ''
summary_en: Flock is a company providing AI surveillance systems used by law enforcement in various US
  locations. This event concerns political and social controversy around the system, representing industry
  regulation and policy context.
inspiration_en: ''
priority_review: false
project_type: new_application
industries: []
industries_en: []
jobs: []
jobs_en: []
regions: []
regions_en: []
open_source: false
url: https://www.ft.com/content/b207536e-6def-4080-878f-d0eba61d11e0
canonical_url: https://ft.com/content/b207536e-6def-4080-878f-d0eba61d11e0
summary: US Republicans revolt against Flock AI surveillance as backlash intensifies
first_seen: '2026-09-07T04:07:07Z'
last_seen: '2026-09-25T00:34:10Z'
status: pending_filter
sources:
- hackernews
- marketfeeds
sightings:
- source: hackernews
  url: https://www.ft.com/content/b207536e-6def-4080-878f-d0eba61d11e0
  seen_at: '2026-09-08T14:34:23Z'
  metrics:
    points: 41
    comments: 5
  kind: news
- source: marketfeeds
  url: https://www.theverge.com/policy/1000005/flock-senate-hearing
  seen_at: '2026-09-25T00:34:10Z'
  metrics: {}
  kind: news
---

# Flock

US Republicans revolt against Flock AI surveillance as backlash intensifies

## 笔记


