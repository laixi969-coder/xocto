---
slug: onecli-yc-s26
name: OneCLI (YC S26)
builder: guyb3
category: AI + 开发
summary_zh: 面向团队的开源沙盒代理 harness
inspiration: 沙盒代理 harness 可能提高团队 AI 开发的安全性
summary_en: OSS sandboxed agent harness for teams
inspiration_en: Sandboxed agent harnesses could improve team AI development safety
priority_review: false
url: https://github.com/onecli/onecli
canonical_url: https://github.com/onecli/onecli
summary: OSS sandboxed agent harness for teams
first_seen: '2026-08-19T16:29:02Z'
last_seen: '2026-08-21T22:41:50Z'
status: queued
sources:
- hackernews
- producthunt
sightings:
- source: hackernews
  url: https://github.com/onecli/onecli
  seen_at: '2026-08-20T22:45:15Z'
  metrics:
    points: 85
    comments: 25
- source: producthunt
  url: https://www.producthunt.com/products/onecli
  seen_at: '2026-08-21T22:41:50Z'
  metrics: {}
---

# OneCLI (YC S26)

OSS sandboxed agent harness for teams

## 笔记


