---
slug: deepseek-harness-desktop
name: deepseek-harness-desktop
builder: anywhere-labs
category: AI + 开发
summary_zh: DeepSeek Harness生态的桌面客户端
inspiration: 为开源工具链提供现代化桌面入口，降低使用门槛
summary_en: Desktop client for the DeepSeek Harness ecosystem.
inspiration_en: Providing a modern desktop entry for open-source toolchains lowers the barrier.
priority_review: false
url: https://dshdesktop.cn
canonical_url: https://dshdesktop.cn
summary: 为 DeepSeek Harness (DSH) 生态打造的现代化桌面端体验
first_seen: '2026-08-13T16:30:50Z'
last_seen: '2026-08-19T22:41:22Z'
status: queued
sources:
- github
sightings:
- source: github
  url: https://dshdesktop.cn
  seen_at: '2026-08-19T22:41:22Z'
  metrics:
    stars: 15118
    forks: 713
    open_issues: 170
- source: github
  url: https://github.com/ChisaAlter/Deepseek-Harness-Desktop
  seen_at: '2026-08-14T22:40:51Z'
  metrics:
    stars: 64
    forks: 7
    open_issues: 2
- source: github
  url: https://github.com/hairyf/deepseek-harness-desktop
  seen_at: '2026-08-19T22:41:22Z'
  metrics:
    stars: 601
    forks: 36
    open_issues: 9
- source: github
  url: https://github.com/vibeinging/deepseek-harness-desktop-app
  seen_at: '2026-08-19T22:41:22Z'
  metrics:
    stars: 491
    forks: 6
    open_issues: 6
---

# deepseek-harness-desktop

为 DeepSeek Harness (DSH) 生态打造的现代化桌面端体验

## 笔记


